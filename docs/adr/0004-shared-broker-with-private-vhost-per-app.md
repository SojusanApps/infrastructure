...
`mygamelist-backend` ran its own standalone RabbitMQ instance in its own repo, for its own Celery queues, separate from the broker introduced in [0003](./0003-shared-rabbitmq-broker-for-user-deletion-propagation.md) for deletion propagation. We consolidated both onto a single Shared Broker instance owned by this repo, rather than keeping one broker per Consuming App.
